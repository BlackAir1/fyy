...
public class BookController {
	...
	public String addGoods(MultipartFile picture,Book book,Model m) throws IllegalStateException, IOException{
		...
		//3. 将图片保存到硬盘
		picture.transferTo(new File("C:\\Users\\Administrator\\Desktop\\Webfinal\\SpringMVCMyBatisGradle\\src\\main\\webapp\\images\\" + newfileName));
		//4.将图片名称保存到数据库
	...
	public String updateGood(Book book,MultipartFile pictureFile) throws IllegalStateException, IOException {
		...
		String newfileName = UUID.randomUUID().toString() + fileStr.substring(fileStr.lastIndexOf("."));
		//3. 将图片保存到硬盘
		pictureFile.transferTo(new File("C:\\Users\\Administrator\\Desktop\\Webfinal\\SpringMVCMyBatisGradle\\src\\main\\webapp\\images\\" + newfileName));
		//4.将图片名称保存到数据库
		book.setImage(newfileName);
